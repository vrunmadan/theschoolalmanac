...
async function rehostPhotos(rows) {
    ...
    if (supaUrl && supaKey) {
          ...
          bucket = createClient(supaUrl, supaKey, { auth: { persistSession: false } }).storage.from('school-photos');
    } else {
  ...
  for (const row of rows) {
        ...
        for (const [i, f] of raw.entries()) {
          ...
          if (!bucket) continue;                                 // can't rehost this run — drop, don't ship broken
          try {
                    ...
                    const { error } = await bucket.upload(objectPath, buf, {
                                upsert: true,
                                contentType: res.headers.get('content-type') || undefined,
                    });
                    if (error) throw error;
                    out.push(bucket.getPublicUrl(objectPath).data.publicUrl);
          } catch (e) {
