...
const SCHOOLS = raw.map((s, i) => ({
  ...
  // Fees in the Notion import were compiled from third-party sources. We do NOT
  // publish those as fact. A fee appears on a page only once a school or parent
  // verifies it through the claim/fee flow (Supabase, source-tagged). Strip the
  // imported figures so nothing unverified is ever shown as a fee.
  tuition: null,
  tuition_num: null,
}));
